...
// time. The loader must keep one record for k.ts: both importers see the same namespace, and the outer fetch does
// not settle the entry a second time (a debug build asserts on that). Two shapes: the pending fetch belongs to a
// dependency edge (a.ts imports k.ts, so k.ts has a registry entry while its fetch is pending) or to the top-level
// import() of k.ts itself (no entry exists until the require() creates one).
          ...
          Bun.plugin({
            ...
            setup(build) {
              build.onLoad({ filter: /[\\\\/]k\\.ts$/ }, () => {
                ...
                if (calls === 2) globalThis.n = require("./n.ts");
                return { contents: "export const k = " + calls + ";", loader: "ts" };
              });
            },
          });
