...
fn create_parsed_shell_script_impl(
    ...
    // Ownership transfers to the C++ wrapper (stored opaquely, freed via the
    // generated finalize callback).
    let this_jsvalue = ParsedShellScript__createWithValues(
        global,
        bun_core::heap::into_raw(parsed_shell_script).cast::<core::ffi::c_void>(),
        marked_argument_buffer,
    );
