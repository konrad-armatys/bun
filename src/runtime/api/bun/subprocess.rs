...
impl Subprocess<'_> {
    ...
    pub(crate) fn get_stderr<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_stdin<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_stdout<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_terminal<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_connected<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_pid<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        Ok(scope.number(this.pid() as f64))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_killed<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        Ok(scope.boolean(this.has_killed()))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_stdio<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let global = scope.unscoped_global();
...
pub mod testing_apis {
    ...
    pub(crate) fn inject_stdio_read_error<'s>(
        scope: &mut Scope<'s>,
        callframe: &CallFrame,
    ) -> JsResult<Local<'s>> {
        ...
        let kind_str = kind_value.to_bun_string(scope)?;
