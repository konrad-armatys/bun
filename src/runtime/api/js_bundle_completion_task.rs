...
bun_event_loop::task_hop! {
    /// `task_tag::JSBundleCompletionTask`: the bundle thread posted the
    /// outcome. `build_ref` keeps the task alive from `schedule` until
    /// `on_complete` releases it. A build handed back unrun during teardown
    /// (cancelled in the stop phase) releases the keep-alive, plugin cell and
    /// promise against the live heap without delivering.
    pub CompletionHop for JSBundleCompletionTask => JSBundleCompletionTask;
    run = JSBundleCompletionTask::on_complete;
    release_unrun = |this| { let _ = JSBundleCompletionTask::on_complete(this); };
}
