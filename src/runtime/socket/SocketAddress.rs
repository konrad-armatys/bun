...
use bun_jsc::{
    CallFrame, JSGlobalObject, JSValue, JsClass, JsError, JsResult, Local, Scope, StringJsc,
};
use bun_ptr::JsCell;
...
impl SocketAddress {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_family<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let global = scope.unscoped_global();
    ...
    /// `sockaddr.addrfamily`
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_addr_family<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_flow_label<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
