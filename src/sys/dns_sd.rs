...
impl Connection {
    ...
    /// Start a `kDNSServiceFlagsShareConnection` address query whose replies go
    /// to `*context`. The holder of `context` keeps it alive for as long as the
    /// returned [`Query`].
...
impl Drop for Query {
    fn drop(&mut self) {
        // SAFETY: the live subordinate ref `DNSServiceGetAddrInfo` returned.
        unsafe { DNSServiceRefDeallocate(self.0.as_ptr()) }
    }
}
