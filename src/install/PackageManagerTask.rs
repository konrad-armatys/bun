    ...
    fn run_owned(mut self: Box<Self>) {
        ...
        'body: {
            match &mut this.request {
                ...
                Request::GitClone(req) => {
                    let name = req.name.slice();
                    let url = req.url.slice();
                    let cache = crate::repository::GitCache::of(manager);
                    let mut url_buf = bun_paths::PathBuffer::uninit();
                        ...
                        if let Some(https) = Repository::try_https(url, &mut url_buf) {
                            match Repository::download(
                                ...
                                Err(err) => {
                                    this.err = Some(err);
                                    this.status = Status::Fail;
                                    this.data = Data::GitClone(Fd::invalid());
                    ...
                    this.err = None;
                    this.data = Data::GitClone(dir.into_raw());
                    this.status = Status::Success;
                }
                Request::GitCheckout(git_checkout) => {
                    match Repository::checkout(
                        git_checkout.env,
                        &mut this.log,
                        crate::repository::GitCache::of(manager),
