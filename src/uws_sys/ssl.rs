...
unsafe extern "C" {
    ...
    fn us_internal_ssl_loop_state_save(ssl: *const SSL, out5: *mut *mut c_void);
    fn us_internal_ssl_loop_state_restore(saved5: *mut *mut c_void);
...
pub struct SslLoopState([*mut c_void; 5]);

impl SslLoopState {
    /// A snapshot that restores nothing.
    pub const fn none() -> Self {
        SslLoopState([core::ptr::null_mut(); 5])
    }
    ...
    pub fn save<const IS_SSL: bool>(socket: &NewSocketHandler<IS_SSL>) -> Self {
        ...
        if let InternalSocket::Connected(_) = socket.socket {
            if let Some(ssl) = socket.ssl_mut() {
                ...
                // loop BIO whose data is `loop_ssl_data`; `saved` has 5 slots.
