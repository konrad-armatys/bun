...
/// calls `deallocator(ptr, deallocator_context)` exactly once: when the
/// object is collected (possibly on another thread, if the buffer was
/// transferred), or on this thread before this returns `Err` (a `len` above
/// `MAX_ARRAY_BUFFER_SIZE` is a RangeError). Never, if `deallocator` is
/// `None`.
///
/// # Safety
///
...
///   `(ptr, deallocator_context)`, possibly on another thread, and
